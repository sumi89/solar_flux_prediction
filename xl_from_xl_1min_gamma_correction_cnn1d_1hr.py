...
print('y_seq_tr_shape', y_seq[train].shape)
print('y_seq ts shape : ', y_seq[test].shape)
...
model.add(layers.Conv1D(32, 7, activation='relu', input_shape = (x_seq.shape[1], n_features)))
...
model.add(layers.Dense(y_seq.shape[1]))
model.summary()

model.compile(optimizer=Adam(lr=lr, clipnorm=cn), loss='mse')

# fit model
history = model.fit(x_seq[train], y_seq[train],  batch_size=batch_size, epochs=epochs, verbose=1,
          validation_data=(x_seq[test], y_seq[test]))
...
model.save('/data/sumi/gamma_correction/cnn_model_pred/cnn_model/'+model_name+'_'+str_batch+'_'+str_lr+'_'+str_look+'_'+str_delay+'.hdf5')
np.save('/data/sumi/gamma_correction/cnn_model_pred/cnn_pred/'+model_name+'_'+str_batch+'_'+str_lr+'_'+str_look+'_'+str_delay+'.npy', prediction.astype(np.float32))
